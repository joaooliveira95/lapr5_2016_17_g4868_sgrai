...
{
	int j = nx +  MAZE_WIDTH/ 2.0 +0.5;
	int i = nz + MAZE_HEIGHT / 2.0 +0.5;
...
void desenhaCubo()
{
	...
	{ 0.5,0.5,0.5 },
	{ -0.5,0.5,0.5 } };
	GLfloat normais[][3] = { { 0,0,-1 },{0,0,1},{1,0,0},{-1,0,0},{0,-1,0},{0,1,0}
	...
	desenhaPoligono(vertices[2], vertices[3], vertices[7], vertices[6], normais[5]);
	desenhaPoligono(vertices[3], vertices[0], vertices[4], vertices[7], normais[3]);
	desenhaPoligono(vertices[6], vertices[5], vertices[1], vertices[2], normais[2]);
	desenhaPoligono(vertices[4], vertices[5], vertices[6], vertices[7], normais[1]);
	desenhaPoligono(vertices[5], vertices[4], vertices[0], vertices[1], normais[4]);
...
void Timer(int value)
{
	...
	if (estado.teclas.left) {
		...
		modelo.objecto.dir+=M_PI/16;
		estado.camera.dir_long += M_PI / 16;
	}
	...
	//modelo.homer[JANELA_NAVIGATE].GetSequence(3);  //le Sequencia usada pelo homer
