...
GLboolean inAreaNo(Ponto no, GLfloat ny, GLfloat nx, int i) {
	...
	//Valida se o Homer está dentro do nó de raio = largura, centrado em (X,y)
	//Se estiver devolve True e atualiza nextZ ( altura do Homer)
	if (pow(nx - x, 2) + pow(ny - y, 2) <= pow(largura*5, 2)) {
...
void timer(int value) {
	...
	float velocidade = modelo.objecto.vel*(curr - modelo.prev)*0.01;
	...
	if (estado.teclas.up) {
		if (modelo.objecto.vel<0.5) modelo.objecto.vel += 0.0005;
		teclaUpDown(1);
		andar = GL_TRUE;
	}
